...
<form action="${pageContext.request.contextPath}/modulename/${myFunction eq 'edit' ? 'edituser':'adduser'}" method="POST">
...
<table border="2">
	...
	<c:forEach items="${userList}" var="user">
		<tr>
			...
			<td>${user.uid}</td>
			<td><a href="${pageContext.request.contextPath}/modulename/loadedit?userid=${user.uid}">${user.fname}</a> </td>
			<td>${user.lname}</td>
			<td>${user.age}</td>
		</tr>
	</c:forEach>
